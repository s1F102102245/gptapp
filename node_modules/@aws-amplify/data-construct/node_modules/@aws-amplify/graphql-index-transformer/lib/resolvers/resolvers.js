...
const replaceDdbPrimaryKey = (config, ctx) => {
    ...
    const tableProvisionStrategy = (0, graphql_transformer_core_1.getDatasourceProvisionStrategy)(ctx, object.name.value);
    const useAmplifyManagedTableResources = tableProvisionStrategy
        ? tableProvisionStrategy === "AMPLIFY_TABLE"
        : false;
...
const appendSecondaryIndex = (config, ctx) => {
    ...
    if ((0, graphql_transformer_core_2.isRDSModel)(ctx, object.name.value)) {
        return;
    }
    const table = getTable(ctx, object);
    const keySchema = getDdbKeySchema(config);
...
const overrideIndexAtCfnLevel = (ctx, typeName, table, indexInfo) => {
    const tableProvisionStrategy = (0, graphql_transformer_core_1.getDatasourceProvisionStrategy)(ctx, typeName);
    const useAmplifyManagedTableResources = tableProvisionStrategy
        ? tableProvisionStrategy === "AMPLIFY_TABLE"
        : false;
    if (!useAmplifyManagedTableResources) {
...
const updateResolversForIndex = (config, ctx, resolverMap) => {
    ...
    const deleteResolver = (0, exports.getResolverObject)(config, ctx, 'delete');
    const syncResolver = (0, exports.getResolverObject)(config, ctx, 'sync');
    const dbType = (0, exports.getDBType)(ctx, object.name.value);
...
const makeQueryResolver = (config, ctx, dbType) => {
    ...
    const dbInfo = (0, exports.getDBInfo)(ctx, modelName);
...
const getDBInfo = (ctx, modelName) => {
    const dbInfo = ctx.modelToDatasourceMap.get(modelName);
    const result = dbInfo !== null && dbInfo !== void 0 ? dbInfo : { dbType: graphql_transformer_core_1.DDB_DB_TYPE, provisionDB: true, provisionStrategy: "DEFAULT" };
    return result;
};
exports.getDBInfo = getDBInfo;
const getDBType = (ctx, modelName) => {
    const dbInfo = (0, exports.getDBInfo)(ctx, modelName);
    const dbType = dbInfo ? dbInfo.dbType : graphql_transformer_core_1.DDB_DB_TYPE;
    return dbType;
};
