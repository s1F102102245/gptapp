...
class AppSyncModelJavaVisitor extends appsync_visitor_1.AppSyncModelVisitor {
    ...
    generateCopyOfBuilderClass(model, classDeclaration, isIdAsModelPrimaryKey = true) {
        ...
        const invocations = stepBuilderInvocation.length === 0 ? '' : ['super', (0, visitor_plugin_common_1.indentMultiline)(stepBuilderInvocation.join('\n')).trim(), ';'].join('');
    ...
    generateBuilderMethod(model, classDeclaration, isIdAsModelPrimaryKey = true) {
        const requiredFields = this.getWritableFields(model).filter(field => !field.isNullable && !(isIdAsModelPrimaryKey && this.READ_ONLY_FIELDS.includes(field.name)));
        const types = this.getTypesUsedByModel(model);
