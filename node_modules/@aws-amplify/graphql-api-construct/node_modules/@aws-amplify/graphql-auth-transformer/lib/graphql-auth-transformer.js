...
class AuthTransformer extends graphql_transformer_core_1.TransformerAuthBase {
    constructor() {
        ...
        this.object = (def, directive, context) => {
            ...
            const getAuthRulesOptions = (0, lodash_1.merge)({ isField: false }, (0, graphql_transformer_core_1.generateGetArgumentsInput)(context.transformParameters));
            this.rules = (0, utils_1.getAuthDirectiveRules)(new graphql_transformer_core_1.DirectiveWrapper(directive), getAuthRulesOptions);
            (0, utils_1.validateRules)(this.rules, this.configuredAuthProviders, def.name.value);
        ...
        this.field = (parent, field, directive, context) => {
            ...
            const getAuthRulesOptions = (0, lodash_1.merge)({ isField: true }, (0, graphql_transformer_core_1.generateGetArgumentsInput)(context.transformParameters));
        ...
        this.protectRelationalResolver = (ctx, def, typeName, field, fieldRoles) => {
            ...
            if (this.authModelConfig.has(relatedModelObject.name.value)) {
                ...
                relatedAuthExpression = this.getVtlGenerator(ctx, def.name.value).generateAuthExpressionForRelationQuery(ctx, def, field, relatedModelObject, this.configuredAuthProviders, roleDefinitions, (_a = relatedModelObject.fields) !== null && _a !== void 0 ? _a : []);
            }
            else {
                relatedAuthExpression = this.getVtlGenerator(ctx, def.name.value).generateSandboxExpressionForField(ctx.transformParameters.sandboxModeEnabled);
            }
            if (fieldRoles) {
                const roleDefinitions = fieldRoles.map((r) => this.roleMap.get(r));
                const hasSubsEnabled = ((_b = this.modelDirectiveConfig.get(typeName).subscriptions) === null || _b === void 0 ? void 0 : _b.level) === 'on';
                ...
                fieldAuthExpression = this.getVtlGenerator(ctx, def.name.value).generateAuthExpressionForField(this.configuredAuthProviders, roleDefinitions, (_c = def.fields) !== null && _c !== void 0 ? _c : [], undefined);
            }
        ...
        this.protectUpdateResolver = (ctx, def, typeName, fieldName, acm) => {
            ...
            const dataSource = ((0, graphql_transformer_core_1.isRDSModel)(ctx, def.name.value)
            ...
            fields.forEach((field) => {
                ...
                directives.forEach((directive) => {
                    ...
                    if (directive.name.value === 'hasOne' ||
                        ...
                        if (!(0, graphql_transformer_core_1.isRDSModel)(ctx, def.name.value)) {
        ...
        this.getVtlGenerator = (ctx, typename, field) => {
            ...
            if ((0, graphql_transformer_core_1.isRDSModel)(ctx, typename)) {
                return new rds_vtl_generator_1.RDSAuthVTLGenerator();
            }
    ...
    doesTypeHaveRulesForOperation(acm, operation) {
        const rolesHasDefaultProvider = (roles) => roles.some((r) => this.roleMap.get(r).provider === this.configuredAuthProviders.default);
