...
const constructTransform = (config) => {
    const { transformersFactoryArgs, authConfig, resolverConfig, userDefinedSlots, stackMapping, transformParameters, sqlLambdaVpcConfig, rdsLayerMapping, sqlLambdaProvisionedConcurrencyConfig, } = config;
...
const executeTransform = (config) => {
    const { schema, modelToDatasourceMap, customSqlDataSourceStrategies, datasourceSecretParameterLocations, printTransformerLog, rdsLayerMapping, scope, nestedStackProvider, assetProvider, synthParameters, customQueries, parameterProvider, } = config;
